...
public class PageinationAnnotaionPlugin extends PluginAdapter {
...
//		interfaze.addMethod(m1);
	...
	private void ibatisPageGenerate(Interface interfaze, TopLevelClass topLevelClass, IntrospectedTable introspectedTable){
		...
		if(template != null && template.trim().length()>0){
			String ibatisTemplateClazz = "com.fosun.base.ibatisEx.IbatisTemplate";
			FullyQualifiedJavaType superType = new FullyQualifiedJavaType(ibatisTemplateClazz);
			topLevelClass.addImportedType(superType);
			...
			templateField.setType(new FullyQualifiedJavaType(ibatisTemplateClazz));
			topLevelClass.addField(templateField);
			
			Method getTemplate = new Method();
			getTemplate.setName("getSqlMapClientTemplate");
			getTemplate.setReturnType(new FullyQualifiedJavaType(ibatisTemplateClazz));
